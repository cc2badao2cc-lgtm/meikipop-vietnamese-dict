...
def load_freq_map_from_zip(zf: zipfile.ZipFile) -> dict:
    ...
    freq: dict = {}
    for name in sorted(zf.namelist(), key=_bank_sort_key):
...
def _bank_sort_key(name: str) -> tuple:
    match = re.search(r'_(\d+)\.json$', os.path.basename(name))
    return (name, -1) if not match else (name.rsplit('_', 1)[0], int(match.group(1)))
...
def _normalise_pos(rules_str: str, term: str, reading: str) -> list:
    pos = [r for r in rules_str.split() if r]
    if 'vs' in pos and 'vs-i' not in pos:
        pos.append('vs-i')
    if 'v5' not in pos:
        return pos

    last = reading[-1:] or term[-1:]
    v5_tags = {
        'う': 'v5u',
        'く': 'v5k',
        'ぐ': 'v5g',
    ...
    specific = v5_tags.get(last)
    if specific == 'v5k' and term in {'行く', '逝く', '往く'}:
        specific = 'v5k-s'
    if specific and specific not in pos:
        pos.append(specific)
    return pos
...
def build_from_zip(
    ...
    for seq, group_rows in seq_groups.items():
        redirect_target_seq = abs(seq) if seq < 0 and abs(seq) in seq_groups else None
        if redirect_target_seq is not None:
            entry_id = id_base + redirect_target_seq
        else:
            entry_id = id_base + (ID_NAMESPACE + seq) if seq < 0 else id_base + seq
        first_row = group_rows[0]
        canon_term = first_row[0]
        canon_read = first_row[1]
        redirect_display_term = (
            seq_groups[redirect_target_seq][0][0]
            if redirect_target_seq is not None else None
        )

        senses = []
        sense_rows = seq_groups[redirect_target_seq] if redirect_target_seq is not None else group_rows
        for row in sense_rows:
            ...
            pos = _normalise_pos(rules_str, term, reading)
            senses.append({'glosses': glosses, 'pos': pos, 'tags': tags})
            # JMdict Yomitan exports repeat the same sequence for alternate
            # spellings/readings. Keep one definition payload for the popup;
            # all forms are still indexed below.
            break
        ...
        for row in group_rows:
            ...
            if _has_kanji(term) and term not in seen_terms:
                ...
                display_term = redirect_display_term or term
                lookup_map[term].append((display_term, display_read, freq, entry_id))

            surface_kana = reading if reading else term
            if not reading and _has_kanji(term):
                continue
            if surface_kana not in seen_readings:
                seen_readings.add(surface_kana)
                if reading:
                    freq = get_freq(surface_kana, reading)
                    display_term = redirect_display_term or canon_term
                    lookup_map[surface_kana].append((display_term, reading, freq, entry_id))
                else:
                    freq = get_freq(term, '')
